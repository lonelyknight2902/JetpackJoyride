...
class RoomMap extends Phaser.GameObjects.Container {
    ...
    constructor(scene: PlayScene, x: number, y: number) {
        super(scene, x, y)
        this.map = scene.make.tilemap({ key: 'roomMap' })
        this.width = this.map.widthInPixels
        const backgroundHallwayTileset = this.map.addTilesetImage('room1FG_1_TVOS', 'room1')
        const backgroundHallwayTileset2 = this.map.addTilesetImage('room1FG_2_TVOS', 'room2')
        ...
        this.coins = this.map.createFromObjects('Coin', { gid: 2049, key: 'coin' })
